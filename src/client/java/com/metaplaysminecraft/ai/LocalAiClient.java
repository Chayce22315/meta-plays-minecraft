...
public final class LocalAiClient {
    ...
    public CompletableFuture<AiAction> decide(String perception) {
        String body = "{" +
                "\"model\":\"meta-plays-minecraft\"," +
                "\"messages\":[" +
                "{\"role\":\"system\",\"content\":\"You control a Minecraft player. Return ONLY one JSON action. Never target, attack, or harm players. Hostile mobs are valid combat targets. Valid types: noop, look, move, jump, chat, attack.\"}," +
    ...
    private static AiAction extractAction(String response) {
        try {
            com.google.gson.JsonObject root = com.google.gson.JsonParser.parseString(response).getAsJsonObject();
